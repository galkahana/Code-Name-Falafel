...
namespace Hummus
{
	...
	public:
		...
		bool CanSkipTo(LongFilePositionType inPositionInStream);

		// will skip if can, which is either if not passed position (check with CanSkipTo), or if while skipping will hit EOF
		void SkipTo(LongFilePositionType inPositionInStream);
		
		// will skip by, or hit EOF
		void SkipBy(LongFilePositionType inAmountToSkipBy);
	...
	private:
		IByteReader* mStream;
		LongFilePositionType mAmountRead;
	};
}